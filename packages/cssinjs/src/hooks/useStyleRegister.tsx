...
import { canUseDom, removeCSS, updateCSS } from 'sc-util';
// @ts-ignore
...
import { useContext, JSXElement, Accessor, createMemo } from 'solid-js';
...
export default function useStyleRegister(
  ...
  if (process.env.NODE_ENV !== 'production' && context().mock !== undefined) {
    isMergedClientSide = context().mock === 'client';
  }
  ...
  const globalCache = useGlobalCache(
    ...
    () => {
      ...
      const { hashId, layer, path, nonce } = info();
      const { hashPriority, transformers, linters, container, cache } = context();
  ...
  return (node: JSXElement) => {
    ...
    const { ssrInline, defaultCache } = context();
    if (!ssrInline || isMergedClientSide || !defaultCache) {
      styleNode = <Empty />;
    } else {
      const [cachedTokenKey, cachedStyleId, cachedStyleStr] = globalCache();
      styleNode = (
        <style
          {...{
